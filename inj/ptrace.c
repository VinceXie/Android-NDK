...
void ptrace_write(int pid, unsigned long addr, void *vptr, int len) {
	...
	while (count < len) {
		...
		word = ptrace(PTRACE_POKETEXT, pid, (void*) (addr + count),
				(void*) word);
		count += 4;
...
void *ptrace_dlopen(pid_t pid, const char *filename, int flag) {
	...
	regs.ARM_r0 = (long) ptrace_push(pid, &regs, (void*) filename,
			strlen(filename) + 1);
	regs.ARM_r1 = flag;
...
void *ptrace_dlsym(pid_t pid, void *handle, const char *symbol) {
	...
	regs.ARM_r1 = (long) ptrace_push(pid, &regs, (void*) symbol,
			strlen(symbol) + 1);
...
#define ARGS_MAX 64
	...
	for (i = 0; i < argc; i++) {
		...
		if (arg->type == PAT_STR) {
			...
			arg->_stackid = ptrace_push(pid, &regs, arg->mem.addr,
					arg->mem.size);
		}
	}
...
static Elf32_Addr get_linker_base(int pid, Elf32_Addr *base_start,
		Elf32_Addr *base_end) {
